# -*- coding: utf-8 -*-
"""main_libraryReducing.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1HNjQDnjv5vRCKAtSnob414Zj_5WUxKLj
...
from google.colab import files #should be disabled when streamlit publishing
...
'''# Project to a suitable projected CRS (e.g., UTM zone appropriate for your data)
# Replace 'epsg:XXXX' with the appropriate EPSG code for your desired projection
projected_gdf = selected_polygons_gdf.to_crs('epsg:4326')

# Create a GeoDataFrame with the centroids of the grid cells in the projected CRS
centroid_gdf = projected_gdf.copy()
centroid_gdf['geometry'] = centroid_gdf['geometry'].centroid
...
projected_crs = input("Enter the desired projected CRS (e.g., EPSG:32645 ): ")
...
'''# prompt: download zip file command needed.

#from google.colab import files

def zip_shapefile(shapefile_name):
  """Zips the ESRI shapefile and associated files for download."""
  try:
    with zipfile.ZipFile(f"{shapefile_name}.zip", 'w', zipfile.ZIP_DEFLATED) as zipf:
      zipf.write(f"{shapefile_name}.shp")
      zipf.write(f"{shapefile_name}.shx")
      zipf.write(f"{shapefile_name}.dbf")
      zipf.write(f"{shapefile_name}.prj")
    files.download(f"{shapefile_name}.zip")
    print(f"Shapefile '{shapefile_name}.zip' created and ready for download.")
  except FileNotFoundError:
    print(f"Error: One or more shapefile components not found. Make sure '{shapefile_name}.shp', '{shapefile_name}.shx', '{shapefile_name}.dbf', and '{shapefile_name}.prj' exist in the current directory.")
  except Exception as e:
    print(f"An error occurred: {e}")

# Example usage (replace 'result_gdf' with your actual shapefile name)
zip_shapefile("result_gdf")'''
